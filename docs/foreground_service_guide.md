...
이 문서는 안드로이드의 4대 컴포넌트 중 하나인 포어그라운드 서비스를 테스트하는 간단한 앱의 개발 과정을 안내합니다. 이 앱은 MVVM 아키텍처 패턴을 따릅니다.
...
## 2. HomeViewModel.kt 수정 (비즈니스 로직)

- **파일 경로**: `app/src/main/java/com/ezlevup/servicetest/presentation/home/HomeViewModel.kt`
- **주요 기능**:
    - `AndroidViewModel`을 상속받아 구현하며, UI(View)로부터 비즈니스 로직을 분리합니다.
    - `Application`의 `Context`를 사용하여 서비스 시작/중지 로직을 처리합니다.
    - 안드로이드 버전에 따라 `startForegroundService()` 또는 `startService()`를 올바르게 호출하는 로직을 포함합니다.

## 3. HomeScreen.kt 수정 (UI)

- **파일 경로**: `app/src/main/java/com/ezlevup/servicetest/presentation/home/HomeScreen.kt`
- **주요 기능**:
    - Jetpack Compose를 사용하여 UI를 구성합니다.
    - `viewModel()`을 통해 `HomeViewModel`의 인스턴스를 주입받습니다.
    - "서비스 시작"과 "서비스 중지" 버튼 클릭 시, 직접 서비스를 제어하는 대신 `HomeViewModel`의 해당 함수(`startMyService`, `stopMyService`)를 호출합니다.
...
## 5. build.gradle.kts 수정 (의존성 추가)

- **파일 경로**: `app/build.gradle.kts`
- **주요 기능**:
    - `HomeScreen`에서 `viewModel()` 팩토리 함수를 사용하기 위해 `implementation(libs.androidx.lifecycle.viewmodel.compose)` 의존성을 추가합니다.

## 6. 오류 및 해결: `MissingForegroundServiceTypeException`
